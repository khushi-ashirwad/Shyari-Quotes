export const categorydata = [
    {
      "name": "Item 1",
      "description": "Angry",
      "img": "https://www.shutterstock.com/image-vector/smile-icon-template-design-smiling-260nw-1009231444.jpg",
      "buttons": [
        {
          "text": "Button 1",
          "action": "https://example.com/button1-action"
        },
        {
          "text": "Button 2",
          "action": "https://example.com/button2-action"
        },
        {
          "text": "Button 3",
          "action": "https://example.com/button3-action"
        }
      ],
      "isSensitive": true
    },
    {
      "name": "Item 2",
      "description": "Description for Item 2",
      "img": "https://img.freepik.com/premium-vector/smile-icon-smile-logo-vector-design-happy-emoticon-business-funny-design-vector-emoji-happiness_6415-2003.jpg?size=626&ext=jpg",
      "buttons": [
        {
          "text": "Button 1",
          "action": "https://example.com/button1-action"
        },
        {
          "text": "Button 2",
          "action": "https://example.com/button2-action"
        },
        {
          "text": "Button 3",
          "action": "https://example.com/button3-action"
        }
      ],
      "isSensitive": true

    },
    {
        "name": "Item 3",
        "description": "Angry",
        "img": "https://i.pinimg.com/474x/c1/e4/a4/c1e4a4ef4409e8ab9b3b0aeeca6c8ebe.jpg",
        "buttons": [
          {
            "text": "Button 1",
            "action": "https://example.com/button1-action"
          },
          {
            "text": "Button 2",
            "action": "https://example.com/button2-action"
          },
          {
            "text": "Button 3",
            "action": "https://example.com/button3-action"
          }
        ],
        "isSensitive": true

      },
      {
        "name": "Item 4",
        "description": "Description for Item 4",
        "img": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3_Jbxs2_PzQMNo0tzi2OCnBE-tFHr7DceW9T7_9FpcmlEQs6-Khgp54KCAyKo1kWODnQ&usqp=CAU",
        "buttons": [
          {
            "text": "Button 1",
            "action": "https://example.com/button1-action"
          },
          {
            "text": "Button 2",
            "action": "https://example.com/button2-action"
          },
          {
            "text": "Button 3",
            "action": "https://example.com/button3-action"
          }
        ],
        "isSensitive": true

      }
  ];
  